...
# Run a single check (same function used by GitHub Actions)
python3 -c "from monitor import run_once; run_once()"
...
- For each destination: calls Amadeus API → appends record to `prices.json` → checks if Error Fare
- Amadeus auth uses OAuth2 client_credentials (`get_amadeus_token()`); token is valid ~30 min and fetched fresh each polling cycle
...
- Sends HTML-formatted Telegram message with a Skyscanner deep-link booking URL on detection
- Logs to both stdout and `monitor.log`
- `run_once()` is the public entry point used by both the main loop and GitHub Actions
...
**[.github/workflows/monitor.yml](.github/workflows/monitor.yml)** — GitHub Actions schedule
- Runs `run_once()` hourly via cron; stores secrets as GitHub Actions secrets (same names as `.env`)
- Commits updated `prices.json` back to the repo after each run with `[skip ci]` to avoid loops

**[prices.json](prices.json)** — shared state (auto-created by monitor.py)
```json
{
  "Tokyo":  [{"timestamp": "...", "date": "...", "price": 850.0, "route": "JFK→NRT", "depart_date": "...", "carrier": "...", "flight_no": "...", "departs_at": "...", "arrives_at": "..."}],
  "Berlin": [...]
}
```
...
The same five variables must be added as GitHub Actions secrets for the scheduled workflow to work.
...
- `TRIP_DURATION_DAYS = 7` — length of the round trip; return date = depart + this many days
